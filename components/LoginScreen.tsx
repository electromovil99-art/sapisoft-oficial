...
import { 
    ...
    Globe, ShieldAlert, RotateCcw, Info, MapPin, Phone, ExternalLink, Zap
} from 'lucide-react';
...
type PublicPage = 'HOME' | 'PRICING' | 'TERMS' | 'PRIVACY' | 'REFUND';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto px-4">
            {[
                ...
                <div key={plan.name} className={`relative rounded-[2.5rem] p-10 border-2 transition-all hover:-translate-y-2 ${plan.popular ? 'border-primary bg-white dark:bg-slate-800 shadow-2xl shadow-primary/20 scale-105' : 'border-slate-100 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50'}`}>
                    ...
                    <button className={`w-full mt-12 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${plan.popular ? 'bg-primary text-white shadow-xl hover:bg-primary-dark' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200'}`}>
                        Empezar Ahora
                    </button>
                </div>
            ))}
            ...
            <header className="sticky top-0 z-[100] w-full px-4 py-3 sm:px-6 lg:px-8">
                <div className="glass-panel mx-auto max-w-7xl rounded-full px-6 py-3 shadow-sm flex items-center justify-between border border-white/10">
                    <button onClick={() => setCurrentPage('HOME')} className="flex items-center gap-3 group">
                        ...
                        <span className="text-lg font-black tracking-tighter text-slate-800 dark:text-white uppercase">SapiSoft</span>
                    </button>
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={() => setShowLoginModal(true)} className="rounded-full bg-slate-900 dark:bg-white px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-white dark:text-black shadow-xl hover:scale-105 transition-all active:scale-95">Ingresar al ERP</button>
                    </div>
                </div>
            </header>
                    ...
                    <div className="grid md:grid-cols-4 lg:grid-cols-5 gap-12">
                        ...
                        <div>
                            ...
                            <ul className="space-y-4 text-xs font-bold uppercase tracking-widest">
                                ...
                                <li><a href="#" className="text-slate-600 hover:text-primary transition-colors">Características</a></li>
                            </ul>
                        </div>
                        <div>
